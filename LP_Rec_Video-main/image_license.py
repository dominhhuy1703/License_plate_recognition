...
class MainWindow(QtWidgets.QFrame, layout_image.Ui_Frame):
    def __init__(self,*args, **kwargs):
        ...
        self.btn_nhandang.clicked.connect(self.image_license)
    ...
    def image_license(self, img_path):

        # Tiền xử lý ảnh
        # self.cv2_path = imutils.resize(self.cv2_path, width=2000)
        self.cv2_path = cv2.resize(self.cv2_path, dsize=(1600, int(1600/self.cv2_path.shape[1] * self.cv2_path.shape[0])), interpolation=cv2.INTER_CUBIC)
        ...
        if detected == 1:
            for i in screenCnt:
                ...
                for i in char_x:
                    ...
                    cv2.putText(roi, strCurrentChar, (x+20, y + 50), cv2.FONT_HERSHEY_DUPLEX, 2, (255, 255, 0), 3)
    ...
    def info(self, text):
        ...
        in5 = int(in4[0:2])
        self.let_ten.setText('Đỗ Minh Huy')
        lang = {
